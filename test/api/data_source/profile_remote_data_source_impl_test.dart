...
void main() {
  ...
  group("test getProfileData()", () {
    ...
    final dtoResponse = ProfileDummyData.dummyProfileResponseDto;
    final entityResponse = ProfileDummyData.dummyProfileResponseEntity;
    final DioException dioException = ProfileDummyData.dummyDioException;
    final Exception exception = ProfileDummyData.dummyException;

    test("getProfileData() success", () async {
      // Arrange
      when(apiClient.getProfileData()).thenAnswer((_) async => dtoResponse);
      // Act
      final result = await dataSource.getProfileData();
      // Assert
      verify(apiClient.getProfileData()).called(1);
      expect(result, isA<ApiSuccessResult>());
      expect((result as ApiSuccessResult).data.message, entityResponse.message);
    });

    test("getProfileData() DioException", () async {
      // Arrange
      when(apiClient.getProfileData()).thenThrow(dioException);
      // Act
      final result = await dataSource.getProfileData();
      // Assert
      verify(apiClient.getProfileData()).called(1);
    ...
    test("getProfileData() generic Exception", () async {
      // Arrange
      when(apiClient.getProfileData()).thenThrow(exception);
      // Act
      final result = await dataSource.getProfileData();
      // Assert
      verify(apiClient.getProfileData()).called(1);
